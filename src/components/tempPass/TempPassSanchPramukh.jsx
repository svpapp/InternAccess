...
import {sanchPramukhTempPassService} from '../../api/tempPass/tempPassService';

const TempPassSanchPramukh = () => {
  const [userData, setUserData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const getTempPassSanchPramukh = async () => {
    try {
      setLoading(true);
      const data = await sanchPramukhTempPassService();
  ...
  if (userData.length === 0) {
    return (
      <View style={styles.centerContainer}>
        <Text>No Sanch Pramukh found</Text>
      </View>
    );
  }
...
export default TempPassSanchPramukh;
