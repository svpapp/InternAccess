...
import {sankulPramukhTempPassService} from '../../api/tempPass/tempPassService';

const TempPassSankulPramukh = () => {
  const [userData, setUserData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const getTempPassSankulPramukh = async () => {
    try {
      setLoading(true);
      const data = await sankulPramukhTempPassService();
  ...
  if (userData.length === 0) {
    return (
      <View style={styles.centerContainer}>
        <Text>No Sankul Pramukh found</Text>
      </View>
    );
  }
...
export default TempPassSankulPramukh;
