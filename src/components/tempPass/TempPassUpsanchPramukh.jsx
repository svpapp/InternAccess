...
import {upsanchPramukhTempPassService} from '../../api/tempPass/tempPassService';

const TempPassUpsanchPramukh = () => {
  const [userData, setUserData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const getTempPassUpsanchPramukh = async () => {
    try {
      setLoading(true);
      const data = await upsanchPramukhTempPassService();
  ...
  if (userData.length === 0) {
    return (
      <View style={styles.centerContainer}>
        <Text>No Upsanch Pramukh found</Text>
      </View>
    );
  }
...
export default TempPassUpsanchPramukh;
